...
let resLoaderObj = {
    ...
    _PromiseArray(imageArr, callback) {
        ...
        for (let i = 0; i < imageArr.length; i++) {
            ...
            let isImg = isObject(imageArr[i]) && reg.test(imageArr[i].url).toString() || reg.test(imageArr.url);
            let promise = isImg == 'true' && this._loadImage(imageArr[i]) || this._loadAudio(imageArr[i]);
